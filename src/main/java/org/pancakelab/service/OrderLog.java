...
public class OrderLog {
	...
	public static void logAddPancake(Order order, String description, List<PancakeRecipe> pancakes) {
		long pancakesInOrder = getNumberOfPancakesForOrder(order, pancakes);
	...
	private static long getNumberOfPancakesForOrder(final Order order, final List<PancakeRecipe> pancakes) {
		return pancakes.stream().filter(p -> p.getOrderId().equals(order.getId())).count();
	}
}
